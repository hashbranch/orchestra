...
Orchestra runs Symphony unattended against Linear projects. Linear issue state
...
Upstream Symphony already reads Linear blocker relations, but the local behavior
...
- Until upstream Symphony applies this behavior for every active state,
  Orchestra patches the local Symphony checkout during `install-symphony` and
  `run`.

## Consequences

Symphony will leave blocked dependent work idle until its blockers reach a
...
Symphony release enforces blocker filtering across all active states, Orchestra
