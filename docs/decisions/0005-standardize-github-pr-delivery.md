...
`nathaniel-hb` as PR reviewers. Before moving a Linear issue to the configured
complete state, agents must check GitHub PR comments, review threads, status
checks, and automated Gemini review feedback. They must read, evaluate,
incorporate valid changes, reply to comments, and resolve threads when GitHub
allows it.
